...
export class CountryService {
  ...
  getCountriesByDialCode(dialCode: string): Country[] {
    ...
    return this.countries.filter(
      (c) => c.dialCode.replace(/\D/g, '') === clean
    );
  }
  ...
  searchCountries(
    query: string,
    searchBy: string[] = ['name', 'dialCode', 'iso2']
  ): Country[] {
    ...
    return this.countries.filter((country) => {
      ...
      if (
        searchBy.includes('iso2') &&
        country.iso2.toLowerCase().includes(lowerQuery)
      ) {
        return true;
      }
      if (
        searchBy.includes('iso3') &&
   ...
   */
  getCountriesWithSameDialCode(dialCode: string): Country[] {
